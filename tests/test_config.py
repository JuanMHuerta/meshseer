...
def test_settings_defaults():
    ...
    assert settings.meshtastic_host == "localhost"
    ...
    assert settings.retention_packets_days is None
    assert settings.retention_node_metric_history_days is None
    assert settings.retention_traceroute_attempts_days is None
...
def test_settings_empty_retention_values_disable_expiry():
    settings = Settings.from_env(
        {
            "MESHSEER_RETENTION_PACKETS_DAYS": "",
            "MESHSEER_RETENTION_NODE_METRIC_HISTORY_DAYS": "",
            "MESHSEER_RETENTION_TRACEROUTE_ATTEMPTS_DAYS": "",
        }
    )
