...
- expose only `mesh.example.com` through the tunnel
...
```bash
git clone https://github.com/<your-org-or-user>/meshseer.git /opt/meshseer/app
cd /opt/meshseer/app
uv sync --frozen
...
Start from [meshseer.env.example](meshseer.env.example).
...
Copy [meshseer.service](meshseer.service) to `/etc/systemd/system/meshseer.service`, then:
...
Install `cloudflared` using Cloudflare's package instructions for Debian, then create a named tunnel and route `mesh.example.com` to it.

Use [cloudflared-config.yml.example](cloudflared-config.yml.example) as the starting config.
...
cloudflared tunnel ingress rule https://mesh.example.com/api/health
...
2. Add a public hostname:
   `mesh.example.com` -> `http://127.0.0.1:8000`
...
3. `curl https://mesh.example.com/api/health` returns Cloudflare `404`.
4. `curl https://mesh.example.com/api/status` works publicly.
5. `curl https://mesh.example.com/api/admin/health` returns Cloudflare `404`.
6. `curl https://mesh.example.com/docs` returns `404`.
